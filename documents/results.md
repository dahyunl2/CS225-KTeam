...
# bfs.cpp
For BFS, we first make an adjacency list for the airport graph for memory efficiency. This takes O(V+E) where V represents the number of vertices and E represents the number of edges because we have to search all adjacency lists of the head node to each vertex. Then, to find the edge connecting two airports, we should search for the adjacency list of vertices, which will take O(V).

We create traversals to find the shortest routes between the airports with a given departing location and arriving location based on the least amount of connecting flights.
We first create a string vector that covers all of the airports and initialize all airports to not visited. We then do BFS initialization and then iterate through all adjacent airports. We check if the ID of the adjacent airport is shown as not visited yet. We also check the enqueue neighbor airport and set it to visit.

The test case for BFS checks 30 traversed airports starting from John F Kennedy International Airport, and the result is shown below.
<img width="807" alt="Screen Shot 2022-12-10 at 11 37 54 PM" src="https://user-images.githubusercontent.com/90300666/206888254-62cf089b-f2bf-4bf6-a99c-8247935f1bdd.png">
...
<img width="632" alt="Screen Shot 2022-12-10 at 11 38 40 PM" src="https://user-images.githubusercontent.com/90300666/206888270-28cfc39e-fc5e-436c-a765-f7792218dc9a.png">
